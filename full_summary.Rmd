...
day5 <- read_csv(here("post_survey_questionnaires", "ONE ACRE - Post Survey QQ & WTP Game-DC_WIDE.csv"))

day5only <- anti_join(day5, day4, by = "resp_key") %>%
  mutate(day = "day5")
...
day6only <- anti_join(day6, day5, by = "resp_key") %>%
  mutate(day = "day6")
...
full_data <- bind_rows(day1, day2only, day3only, day4only, day5only, day6only, day7only, day8only, day9only)
...
ggplot(interaction_data, aes(x = reorder(enumerator_name_label, -interaction_value), y = interaction_value)) +
  ...
  labs(title = "Interaction Term Values by Enumerator and Treatment",
       subtitle = "All WTP interaction terms compared to Collins",
       caption = "*No interaction term was statistically significant at the 0.05 level",
...
full_board <- read_csv(here("planning_boards", "ONE ACRE - Planning Board-Final.csv"))
```
